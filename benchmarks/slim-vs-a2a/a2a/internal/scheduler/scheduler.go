...
func (s *Scheduler) Run(ctx context.Context) metrics.RunResult {
	...
	cancelTasks := func(taskIDs []string) {
		...
		for _, id := range taskIDs {
			...
			byAgent[task.Agent] = append(byAgent[task.Agent], id)
		}
		startCancel := time.Now()
		var cancelErr error
		for agentID, ids := range byAgent {
			if err := s.client.CancelTasks(ctx, []string{agentID}, ids); err != nil && cancelErr == nil {
				cancelErr = err
			}
		}
		...
		kv := []string{
			fmt.Sprintf("agents=%d", len(byAgent)),
			fmt.Sprintf("tasks=%d", len(taskIDs)),
		}
		if cancelErr != nil {
			kv = append(kv, fmt.Sprintf("err=%v", cancelErr))
		}
		benchlog.Coord(benchlog.ImplA2A, "cancel", cancelErr == nil, duration, kv...)
	}
	...
	handleFailure := func(taskID string, timedOut bool) {
		...
		if err := s.client.NotifyFailure(ctx, agents, taskID); err != nil {
			benchlog.Coord(benchlog.ImplA2A, "notify_failure", false, 0,
	...
	applyContext := func(taskID string) {
		...
		for _, cu := range s.plan.ContextUpdatesAfter(taskID) {
			...
			benchlog.Coord(benchlog.ImplA2A, "context_push", err == nil, d, kv...)
			...
			if strings.Contains(cu.Payload, "sync=") || strings.Contains(cu.Payload, "phase=") {
				...
				benchlog.Coord(benchlog.ImplA2A, "sync_phase", syncErr == nil, syncD, syncKV...)
			}
	...
	launch := func(task plan.Task) {
		...
		go func(task plan.Task) {
			defer wg.Done()
			taskStart := time.Now()
			benchlog.Task(benchlog.ImplA2A, "started", task.ID, task.Agent, 0)
			...
			if abort && states[task.ID] == stateRunning {
				...
				result.TasksCancelled++
				mu.Unlock()
				benchlog.Task(benchlog.ImplA2A, "cancelled", task.ID, task.Agent, taskDuration)
				return
			}
			mu.Unlock()

			if err != nil {
				benchlog.Task(benchlog.ImplA2A, "failed", task.ID, task.Agent, taskDuration,
					fmt.Sprintf("err=%v", err))
			...
			mu.Unlock()
			benchlog.Task(benchlog.ImplA2A, "completed", task.ID, task.Agent, taskDuration)
	...
	benchlog.Coord(benchlog.ImplA2A, "run_finished", result.Success, time.Since(start),
		...
		fmt.Sprintf("sequential_rpcs=%d", stats.SequentialRPCCount),
	)
	return result
}
