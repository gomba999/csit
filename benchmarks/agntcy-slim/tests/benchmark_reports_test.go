...
func buildThroughputModeTable(rows []benchmarkRunResult, mode string) string {
	...
	lines := []string{
		...
		"",
		fmt.Sprintf("Confidence intervals are shown only when a case has at least %d repeated runs.", configuredMinimumConfidenceIntervalRuns()),
		"",
...
func buildCapacitySweepTechnicalSection(cfg suiteConfig, results []capacitySweepCaseResult) string {
	lines := []string{
		...
		fmt.Sprintf("Confidence intervals are only reported when a sweep step has at least %d repeated runs.", configuredMinimumConfidenceIntervalRuns()),
		"",
